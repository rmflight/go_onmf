function [table1 table12 t1 T1 xcg ycg lambda]=kmplot(varargin)
...
default.values = {[7 0; 12 0; 7 1; 12 1; 11 1; 8 0; 9 0; 6 0; 7 1; 2 0],0.05,1};
default.values(1:nu) = args;
[x alpha flag] = deal(default.values{:});
...
a1=[table1(1,2); -1.*table1(2:end,2)]; % Curve
...
model=@(l,x)(exp(-l.*x));
% lambda=nlinfit(t1,T1,model,0.5);
%lambda=nlinfit(t1,T1,model,0.5,statset('robust','on','FunValCheck','off'));
lambda=nlinfit(t1,T1,model,0.5,statset('FunValCheck','off'));
%lambda = 1;
