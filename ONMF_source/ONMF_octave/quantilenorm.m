...
% $Revision: 1.1.12.7 $   $Date: 2008/06/16 16:34:00 $
        ...
        elseif length(k)>1
            ...
            switch(k)
                case 1  % tied flag
                    ...
                    % efficient.
                    tiedFlag = opttf(pval,okargs{k},mfilename);
                case 2 % display flag
                    dispFlag = opttf(pval,okargs{k},mfilename);
                case 3 % median flag
                    medianFlag = opttf(pval,okargs{k},mfilename);
                    if medianFlag
                        avgFcn = @nanmedian;
                    end
            end
...
for col = 1:valSize(2)
    ...
    if (tiedFlag)
        rr{col} = sort(tiedrank(values(~nanvalus(:,col),col)));
    end
...
mean_vals = feval(@median,rankedVals,2);
