...
%      UseParallel - Flag indicating whether eligible functions should use 
%                    capabilities of the Parallel Computing Toolbox (PCT), 
%                    if the capabilities are available.  That is, if the
%                    PCT is installed, and a PCT matlabpool is in effect.
%                    Eligible functions are bootci, bootstrp, crossval,
%                    jackknife, and the TreeBagger constructor.  
%                     Valid values are 'never' (the default), for serial
%                    computation, and 'always', for parallel computation.
%      UseSubstreams-Flag indicating whether the random number generator in
%                    eligible functions should use Substream property of
%                    the RandStream class. 'never' (default) or 'always'.
%                    'always', high level iterations within the function will
%                    set the Substream property to the value of the
%                    iteration.  This behavior helps to generate
%                    reproducible random number streams in parallel and/or
%                    serial mode computation.  Eligible functions are
%                    bootci, bootstrp, crossval, and the TreeBagger constructor.
%      Streams     - A single instance of the RandStream class, or a 
%                    cell array of RandStream instances.  The Streams
%                    option is accepted by some functions to govern
%                    what stream(s) to use in generating random numbers
%                    within the function. If 'UseSubstreams' is 'always',
%                    the Streams value must be a scalar, or must be empty.
%                    If 'UseParallel' is 'always' and 'UseSubstreams' is
%                    'never', then the Streams argument must either be empty,
%                    or its length must match the number of processors used
%                    in the computation: equal to the matlabpool size if a
%                    matlabpool is open, a scalar otherwise.
%      OutputFcn   - Function handle specified using @, a cell array with 
%                    function handles or an empty array (default).  The solver
%                    calls all output functions after each iteration.
%
%   See also STATGET.

%   Copyright 1993-2009 The MathWorks, Inc.
%   $Revision: 1.3.6.22 $  $Date: 2009/05/18 21:17:38 $
...
if nargin == 1
    ...
    if (ischar(arg) || isa(arg,'function_handle'))
        ...
        switch lower(arg)
            ...
            case {'bootci' 'bootstrp' 'crossval' 'treebagger'}
