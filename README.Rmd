...
OK, so we are equally split between having all the same children, and having most
of them. 

Lets check how this works out if we split the other way on everything, and then
get all the ancestors.

```{r onmf_all_relationships}
go1 <- unlist(go_map[match(go2go$GO1, go_map$loc), "GO"], use.names = FALSE)
go2 <- unlist(go_map[match(go2go$GO2, go_map$loc), "GO"], use.names = FALSE)
onmf_child_parent <- split(go1, go2)

go_ancestors <- mget(names(onmf_child_parent), GOBPANCESTOR, ifnotfound = NA)
```
